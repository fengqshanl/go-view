  ...
  const requestIntervalFn = () => {
    ...
    try {
      ...
      if (requestUrl?.value) {
        ...
        // 开启轮询
        // if (time) {
        //   const cycle = window.localStorage.getItem("go-view-recycle") ? JSON.parse(window.localStorage.getItem("go-view-recycle") || "") : []
        //   fetchInterval = setInterval(fetchFn, intervalUnitHandle(time, unit))
        //   cycle.push(fetchInterval)
        //   window.localStorage.setItem("go-view-recycle", JSON.stringify(fetchInterval))
        // }
      }
