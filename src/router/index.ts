...
export function setupRouter(app: App, props: any) {
  app.use(router);

  if (props?.routerControl) {
    props?.routerControl(router, history)
  }
  // if (props.history.initialEntries[0]) {
  //   history.go(props.history.initialEntries[0])
  // }
  // 创建路由守卫
